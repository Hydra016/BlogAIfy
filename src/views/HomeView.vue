...
<template>
  <div class="min-h-screen bg-[#1e1e1e] text-white px-4 py-10">
    <h1 class="text-3xl font-bold mb-8 text-center text-[#FFD11A]">Latest Blog Posts</h1>

    <div v-if="!blogStore.isLoading">
      <div v-if="blogStore.sortedBlogs.length > 0"
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <Card v-for="blog in blogStore.sortedBlogs" :key="blog.blog_id" :blog="blog" :primary="true" />
      </div>
